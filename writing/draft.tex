...
The POPC lipids consist of a choline head group (Q0), a phosphate group (Qa) and two 
glycerol groups (Na) connecting the polar part to two tails. These are made up of a oleoyl (three C1 and one C3)
and a palmitoyl group (four C1) and are highly apolar. The resulting molecule 
is shown in \cref{popc_lammps}.  

\begin{figure}[t]
\centering
\includegraphics[width=10cm]{figures/theory/popc_cg.png}
...
\section{Area per lipid}

Before starting the self-assembly simulation there are many system configurations that need to be 
investigated, starting with the area per lipid. For this a starting configuration is employed where 
every lipid is placed on a grid adjusted to the box size and rotated to start the simulation with a 
bilayer configuration. The relative positions of the beads in the POPC molecule are extracted from 
the \path{POPC-em.gro} and are then divided by the length scale from \cref{dpd_units}. The water beads 
are placed randomly outside of the lipid phase and the water per lipid ratio is chosen as 22:1. This
value was adopted from sucessful experiments. As mentioned earlier the work Groot and Warren (\textbf{cite here})
dictate a number density of $\rho=3$. The area per lipid $A_{\text{lipid}}$ fixes the box dimension of the xy plane 
according to

\begin{align}
    L_x = L_y = \sqrt{A_{\text{lipid}}\cdot N_{\text{lipid}}/2}
\end{align}
